...
class PublisherMW():
    ...
    def bindPub(self):
        ...
        if self.upcall_obj.dissemination == 'Direct':
            ...
            self.logger.info("Choose Direct strategy and bind with port {}".format(bind_string))
        else:
            ...
            connect_str = "tcp://" + str(ip) + ":" + str(int(port) + 1)

            self.logger.info("the path is {}".format(connect_str))
    ...
    def handle_reply(self):
        ...
        try:
            ...
            self.logger.info("PublisherMW::Receive data {}".format(disc_resp))
    ...
    def register(self, name, topiclist):
        ...
        try:
            ...
            self.logger.info("PublisherMW::Receive data {}".format(disc_req))
    ...
    def is_ready(self):
        ...
        try:
            ...
            self.logger.info("PublisherMW::Send data {}".format(disc_req))
            # now let us stringify the buffer and print it. This is actually a sequence of bytes and not
            # a real string
            buf2send = disc_req.SerializeToString()
            self.logger.debug("Stringified serialized buf = {}".format(buf2send))
    ...
    def disseminate(self, id, topic, data):
        try:
            ...
            # str = discovery_pb2.Publication()
            # str.topic = topic
            # str.content = data
            # str.pub_id = id
            # str.tstamp = time.time()
    ...
    #     query = "SELECT * FROM pub_id"
    #     res = requests.get(url, params={'q': query})
    #     if res.status_code != 200:
    #       raise Exception("Query failed")
    #     csv_file = open("timestamp.csv", "w")
    #     csv_write = csv.writer(csv_file)
    #
    #     df.to_csv('pub_sub.csv', index=False)
